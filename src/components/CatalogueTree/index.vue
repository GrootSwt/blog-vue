<template>
  <div class="catalogue-tree">
    ...
    <el-button
      class="folder-btn"
      @click="catalogueTreeVisible = !catalogueTreeVisible"
      circle
      type="info"
    >
      <el-icon>
...
export default {
  ...
  setup (props, { emit }) {
    ...
    const createCatalogue = (createType) => {
      const { parentId: currentParentId } = currentNode
      const { id: currentId } = currentNode
      switch (createType) {
        // 同级文档
        case '1':
    ...
    // 展示目录
    const showCatalogueTree = () => {
      catalogueTreeVisible.value = true
    }
    // 隐藏目录
